...
class AdvancedAnsibleExtractor:
    ...
    def detects_encryption_requirement(self, content: str) -> bool:
        ...
        """
        encryption_indicators = ["ServerSideEncryption", "Encryption", "KmsKeyId", "encrypt"]
    ...
    def get_verifiable_state(self, returns: Dict) -> List[str]:
        ...
        """
        verifiable_keys = ["path", "mode", "uid", "gid", "state", "owner", "group"]
    ...
    def _parse_examples_fallback(self, examples_text: str) -> List[Dict]:
        ...
        for line in examples_text.split("\n"):
            if line.strip().startswith("- name:"):
                ...
                examples.append({"name": name, "module": "unknown", "parameters": {}})
    ...
    def _build_permission_detail(self, permission: str, content: str) -> Dict:
        ...
        detail = {
            "action": permission,
            "resource_constraint": (
                "*" if any(keyword in permission for keyword in ["Describe", "List"]) else None
            ),
        }
        ...
        for match in re.finditer(exception_pattern, content):
            ...
            if fail_matches:
                for fail_match in fail_matches:
                    ...
                    recovery_hint = self._generate_exception_recovery_hint(
                        exception_type, message, error_code
                    )
    ...
    def _extract_specific_exceptions(
        self, block_content: str, exception_type: str
    ) -> List[ErrorPattern]:
    ...
    def _generate_exception_recovery_hint(
        self, exception_type: str, message: str, error_code: Optional[str]
    ) -> str:
