                    ...
                    for match in matches:
                        artifacts.append(
                            CodeArtifact(
                                ...
                                line_number=line_no,
                                is_documented=self._has_jsdoc(lines, line_no - 1)
                                or self._has_comment(lines, line_no - 1),
                            )
                        )
...
class ArtifactCoverageCalculator:
        ...
        for ext, extractor in self.extractors.items():
            for file_path in path.rglob(f"*{ext}"):
                ...
                if any(
                    ...
                    for skip in ["node_modules", "venv", ".venv", "__pycache__", "dist", "build"]
                ):
                    continue
    ...
    def generate_report(self, result: ArtifactCoverageResult) -> str:
        ...
        report = []
        report.append("📊 Artifact-Based Documentation Coverage Report")
        report.append(f"{'=' * 50}")
        ...
        for artifact_type, artifacts in result.artifacts_by_type.items():
            ...
            report.append(
                f"  {artifact_type.capitalize()}: {documented}/{total} ({percentage:.1f}%)"
            )
