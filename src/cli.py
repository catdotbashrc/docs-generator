...
def measure(project_path, output, verbose):
    ...
    console.print(
        f"[bold blue]📊 Measuring documentation coverage for {project_path}[/bold blue]\n"
    )
...
@click.argument("project_path", type=click.Path(exists=True))
@click.option(
    "--min-coverage", "-m", default=0.85, type=float, help="Minimum coverage threshold (0-1)"
)
def assert_coverage(project_path, min_coverage):
    """Assert that documentation meets minimum coverage (fails if not)"""

    console.print(
        f"[bold blue]🎯 Asserting documentation coverage >= {min_coverage:.0%}[/bold blue]\n"
    )
    ...
    try:
        ...
        console.print("[bold green]✅ Coverage assertion passed![/bold green]")
    except AssertionError as e:
        console.print("[bold red]❌ Coverage assertion failed![/bold red]")
        console.print(f"\n{e}")
        exit(1)
...
def demo(project_path):
    ...
    result = coverage.measure(extracted)
    console.print(
        f"   Extracted coverage: {result.overall_coverage:.1%} {'✅' if result.passed else '⚠️'}"
    )
...
@click.option("--output", "-o", help="Output file for results (JSON)")
@click.option(
    "--min-coverage", "-m", default=85.0, type=float, help="Minimum coverage threshold (0-100)"
)
def measure_artifacts(project_path, output, min_coverage):
    ...
    console.print("[bold blue]🎯 Artifact-Based Documentation Coverage Analysis[/bold blue]\n")
    console.print("[dim]Counting functions, classes, methods, and other code artifacts...[/dim]\n")
    ...
    for artifact_type, artifacts in result.artifacts_by_type.items():
        ...
        table.add_row(
            artifact_type.capitalize(), str(documented), str(total), f"{percentage:.1f}%", status
        )
...
def config_coverage(project_path, output, show_all):
    ...
    console.print("[bold blue]🔧 Configuration Documentation Coverage Analysis[/bold blue]")
    console.print(
        "[dim]Scanning for environment variables, connection strings, and configs...[/dim]\n"
    )
    ...
    for config_type, configs in result.configs_by_type.items():
        ...
        if config_type in ["env_var", "connection_string"]:
            ...
            table.add_row(config_type, str(documented), str(total), f"{percentage:.1f}%", status)
    ...
    if not result.passed:
        ...
        console.print(
            f"  • [red]{len(result.undocumented_configs)} configurations[/red] could cause production failures"
        )
        console.print(
            f"  • [red]{len(result.critical_undocumented)} sensitive configs[/red] pose security risks"
        )
