...
class TestDocumentationCoverage:
    ...
    def test_real_ansible_project_coverage(self, coverage_calculator):
        ...
        assert result.overall_coverage == pytest.approx(
            0.379, 0.01
        ...
        assert result.dimension_scores["automation"] == pytest.approx(
            0.7, 0.01
    ...
    def test_coverage_threshold_validation(self, coverage_calculator):
        ...
        minimal_docs = {"dependencies": {"package_manager": "pip"}}  # Minimal data
    ...
    def test_language_specific_requirements(self, coverage_calculator):
        ...
        assert "node_version" in result.missing_elements.get(
            "dependencies", []
...
class TestFalsePositiveScoring:
    ...
    def test_empty_dimension_should_not_score_high(self):
        ...
        assert (
            result.dimension_scores["yearbook"] == 0.0
        ), "Empty yearbook dimension should score 0%"
        assert (
            result.dimension_scores["governance"] == 0.0
        ), "Empty governance dimension should score 0%"
        assert result.dimension_scores["health"] == 0.0, "Empty health dimension should score 0%"
        assert result.dimension_scores["testing"] == 0.0, "Empty testing dimension should score 0%"
    ...
    def test_missing_dimension_should_score_zero(self):
        ...
        coverage = DocumentationCoverage()
        partial_docs = {
            "dependencies": {"runtime_dependencies": ["package1"], "package_manager": "pip"}
        }

        result = coverage.measure(partial_docs)

        assert (
            result.dimension_scores["automation"] == 0.0
        ), "Missing automation dimension should score 0%"
        assert (
            result.dimension_scores["lifecycle"] == 0.0
    ...
    def test_completeness_should_not_default_to_100_percent(self):
        ...
        completeness = coverage._calculate_completeness_coverage(
            spec, {"element1": None}  # Element exists but is empty
        )

        # Bug fixed - empty elements now return 0% completeness
        assert completeness == 0.0, "Fixed: Empty elements correctly return 0% completeness"
    ...
    def test_usefulness_should_not_default_to_100_percent(self):
        ...
        usefulness = coverage._calculate_usefulness_coverage(spec, {})  # Empty data

        # Bug is FIXED! Empty data now correctly returns 0% usefulness
        assert usefulness == 0.0, "Empty data should have 0% usefulness, not 100%"

        # Also test with None value
        usefulness_none = coverage._calculate_usefulness_coverage(
            spec, {"changelog": None}  # Element exists but is None
        )
        assert usefulness_none == 0.0, "None values should have 0% usefulness"

        # Test with empty dict value
        usefulness_empty = coverage._calculate_usefulness_coverage(
            spec, {"changelog": {}}  # Element exists but is empty dict
        )
        assert usefulness_empty == 0.0, "Empty dict values should have 0% usefulness"
...
class TestMissingExtractorDetection:
    ...
    def test_identify_missing_ci_cd_extractor(self):
        ...
        assert "ci_cd_workflows" in result.missing_elements.get(
            "automation", []
        ), "Should identify missing CI/CD workflow extraction"
        assert any(
            "CI/CD" in rec or "workflow" in rec.lower() for rec in result.recommendations
    ...
    def test_identify_language_mismatch_requirements(self):
        ...
        # Currently Python projects are penalized for missing node_version
        assert (
            result.dimension_scores["dependencies"] < 0.8
...
class TestRealProjectIntegration:
    ...
    def test_coverage_calculation_consistency(self):
        ...
        partial_docs = {
            "dependencies": {"package_manager": "pip", "runtime_dependencies": ["package1"]}
        }
        complete_docs = {
            "dependencies": {
                "package_manager": "pip",
        ...
        assert (
            minimal_result.overall_coverage < partial_result.overall_coverage
        ), "Partial docs should score higher than minimal"
        assert (
            partial_result.overall_coverage < complete_result.overall_coverage
