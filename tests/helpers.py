...
class ModuleParser:
    ...
    def extract_fail_json_calls(content: str) -> List[Dict[str, str]]:
        ...
        for match in re.finditer(pattern, content):
            errors.append({"message": match.group(1), "type": "explicit_error"})
...
class CoverageAnalyzer:
    ...
    def calculate_dimension_coverage(
        dimension_data: Dict[str, Any], required_elements: List[str]
    ) -> float:
    ...
    def generate_coverage_report(
        coverage_scores: Dict[str, float], threshold: float = 0.85
    ) -> Dict[str, Any]:
        """Generate detailed coverage report."""
        total_score = sum(coverage_scores.values()) / len(coverage_scores)
        passing_dimensions = {
            dim: score for dim, score in coverage_scores.items() if score >= threshold
        }
        failing_dimensions = {
            dim: score for dim, score in coverage_scores.items() if score < threshold
        }
...
class MockDataFactory:
    ...
    def create_coverage_result(
        overall: float = 0.85, dimensions: Optional[Dict[str, float]] = None
    ) -> Dict[str, Any]:
    ...
    @staticmethod
    def create_extraction_result(module_name: str = "test_module") -> Dict[str, Any]:
        ...
        return {
            ...
            "permissions": ["ec2:DescribeInstances", "ec2:TerminateInstances"],
            "error_patterns": [{"message": "File not found", "recovery": "Check file path"}],
...
class PerformanceValidator:
    ...
    @classmethod
    def validate_batch_performance(cls, module_count: int, elapsed_time: float) -> bool:
...
class TestReporter:
    ...
    @staticmethod
    def generate_test_summary(passed: int, failed: int, skipped: int = 0) -> str:
...
class ValidationHelpers:
    ...
    @staticmethod
    def validate_extraction_completeness(extracted_data: Dict[str, Any]) -> List[str]:
        """Validate that extraction captured required information."""
        missing = []

        required_fields = ["module", "parameters", "permissions", "error_patterns", "states"]
